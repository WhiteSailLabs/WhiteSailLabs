...
> **我的 FDE Build Lab 项目 · 技术设计完成。** 我使用合成食品包装和公开规则实现；项目不构成法律建议或合规认证，范围、数据、工程与评估均由我负责。

[English version](packaging-preflight.md)

## 审核为什么总在最后排队

在我设定的消费品团队场景中，产品从需求、口味、配方到包装、合规、中试和渠道上线，会经过产品、研发、设计、法务、采购和渠道多个角色。系统往往记录最后结果，却没有留下“为什么这样做”的上下文。
...
项目的关键问题不是“能不能让 AI 看图”，而是重新划分工作：哪些确定性问题应该自动拦截，哪些历史反馈应该沉淀，哪些判断必须留给人。
...
我把“发现问题”本身设计为项目能力：使用合成会议记录、驳回说明和设计任务建立一个有边界的观察窗口，判断哪些问题高频、可衡量、值得进入候选清单。观察必须有明确目的、数据范围和结束时间，不能演变为无限采集。

## 第二个关键动作：先写 Agent 岗位说明书

确定包装审核值得做之后，我不会直接写 Skill，而是采用 HRBP 式拆解：输入是什么、输出是什么、哪些规则直接执行、哪些情况需要判断、需要哪些上下文。
...
这是我的公开 Build Lab 项目。我负责合成包装、Agent 岗位说明书、规则系统、评估数据、审核体验和工程实现；任何结果都以可复现基准为准。
