...
> **个人 FDE Build Lab · 原型规格完成。** 本项目受《FDE案例100》案例 10 启发，使用程序生成的 POS、商场结算和票据数据。本文会引用原案例的业务观察，但不会把原案例的客户、收益或上线效果说成我的经历。
...
原案例中的客户是一家年营收数十亿元、门店主要位于百货和商业综合体的消费品企业。总部过去甚至设置“容忍区间”：因为查清小差异的人工成本太高，低于某个比例就不再追。FDE 进场后发现，问题不只是节省人工，而是让企业重新有能力追查过去被成本迫使放弃的差异。部分门店实际差异达到 1%–3%，有少收也有多收，最终要看净差异。

这就是故事的转折：一个“省时间”的后台流程，可能直接靠近收入。

## 为什么这个问题适合作为第一单

客户通常不会只提出一个需求。原案例中同时还有自动客服、竞品对比等方向，但对账被排到前面，因为它具备一个小胜仗需要的条件：流程清晰、涉及角色少、价值可计算、反馈周期短、技术成本可控，而且用户不需要先投入大量时间学习。
...
原案例第一阶段只覆盖上海少量门店，甚至可以用 Cherry Studio 和本地电脑完成；但团队从第一天就考虑几百、上千门店在接近时间窗口运行时的并发、冗余和失败恢复。
...
故事原型来自 [《FDE案例100》案例 10：从人工核对到智能对账](https://assets.datawhale.cn/Datawhale%20FDE%E6%A1%88%E4%BE%8B100.pdf)。本文中的虚拟门店、异常生成器、界面故事和工程计划由我重新构建。
